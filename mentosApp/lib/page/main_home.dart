...
class _MainHomeState extends State<MainHome> {
  ...
  void openCreatePostPage() async {
    ...
    final flaguser =
        FirebaseFirestore.instance.collection('info'); // db에 저장된 게시물들을
    ...
    Timestamp start =
        snapshot.docs[0].data()['startDate_info']; // +버튼에서 시작일을 입력한 뒤
    DateTime start2 = start.toDate();
    String start3 =
        DateFormat('MM/dd/yyyy, hh:mm a').format(start2); // 시작일을 보기쉬운 형식으로 바꿈.

    Timestamp end = snapshot.docs[0].data()['endDate_info']; // 종료일을 입력한 뒤
    DateTime end2 = start.toDate();
    String end3 =
        DateFormat('MM/dd/yyyy, hh:mm a').format(start2); // 종료일을 보기 쉬운 형식으로 바꿈.
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          // 메인페이지에 만들어진 게시물의 + 를 눌렀을 때.
                          final haveto = FirebaseFirestore.instance
                              .collection('users')
                              .doc(loginIdController.text);
                          final snapshot =
                              await haveto.get(); // 지금 사용하는 사용자의 정보를 가져옴.

                          final flaguser =
                              FirebaseFirestore.instance.collection('info');
                          final snapshot2 =
                              await flaguser.get(); // 게시물의 정보도 가져옴.

                          //print(snapshot.data()!['userbirthday']);
                          Timestamp t_birth = snapshot.data()!['userbirthday'];
                          DateTime u_birthday =
                              t_birth.toDate(); // 사용자의 정보를 업데이트할 때 날짜 형식을 다시 세팅

                          Timestamp endDate =
                              snapshot2.docs[0].data()['endDate_info'];
                          DateTime endDate2 = endDate.toDate();
                          //print("u_birthday = $u_birthday");

                          final userinfo = User(
                              // 사용자의 정보에서 빈칸인 나의 스터디에 게시물의 정보를 넣기 위함.
                          ...
                          musthave = snapshot2.docs[0].data()[
                              'title_info']; // 제일 첫번째 게시물의 제목을 musthave에 저장.
                          a_date = DateFormat('MM/dd/yyyy, hh:mm a')
                              .format(endDate2); // 제일 첫번째 게시물의 종료일도 a_date에 저장

                          await haveto.set(userinfo
                              .toJson()); // 사용자의 개인정보 업데이트(나의 스터디가 추가된 사항.)
                        }, // '+' 버튼을 눌렀을 때의 동작 구현
                      ),
                    ],
                  ),
